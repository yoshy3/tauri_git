...
- コミットメッセージを編集してコミットを revert する
- リンクワークツリーの一覧表示・追加・削除。ダブルクリックで切り替え
...
- コミット履歴の検索とフィルタ。フィルタ中は一覧を見やすくするためグラフを非表示にします
- サイドバーでのブランチ検索とフィルタ
...
- コミット詳細からコミットを revert（メッセージ編集ダイアログ付き）
- stash の作成 / 適用 / pop
...
- リンクワークツリーの管理: 一覧表示、指定パス・ブランチへの追加、削除
- Worktrees セクションをサイドバー最上部に固定し、デフォルトで展開表示
- main を含む全ワークツリーをダブルクリックで切り替え
- アクティブなワークツリーをアクセントカラーの左ボーダー・太字・ブランチ名色で強調
- 英語 / 日本語 UI 切り替え
